...
3 бага движка, вскрытые этим аудитом (недостаточность средств/маржи, margin call
фьючерсов, equity при дыре в мульти-тикерной ленте), исправлены в backtest/core.py
2026-07-02 — см. analysis/engine_audit_result.md.
"""
import math
import random
...
def test_futures_margin_call_triggers_before_ruinous_loss_with_realistic_margin():
    """Тест выше проходит и вырожденно: margin_rate=1.0 (дефолт) требует 100% ГО,
    поэтому вход на кэше 100 отклоняется ЕЩЁ ДО крэша (позиция всегда 0) — margin
    call ни разу не срабатывает по факту. Здесь — реалистичный margin_rate=0.1:
    вход легитимно проходит, позиция переживает умеренную просадку, а при более
    глубокой — принудительно закрывается ПОКА equity ещё положительный."""
    inst = {"G": Instrument("G", multiplier=10.0, kind="futures", margin_rate=0.1)}
    b = Broker(2000.0, inst, commission=0.0)
    b.submit(Order("G", 1, MARKET), 0)
    b.process(0, {"G": Bar(0, 1000, 1000, 1000, 1000)})
    assert b.position("G") == 1                       # вход прошёл: ГО 10%×10000=1000 < 2000
    moderate = {"G": Bar(1, 900, 900, 900, 900)}
    b.process(1, moderate)
    assert b.position("G") == 1                        # equity(900) >= ГО(900) — не тронута
    assert b.equity(moderate) > 0
    crash = {"G": Bar(2, 850, 850, 850, 850)}          # equity(500) < ГО(850) — margin call
    b.process(2, crash)
    assert b.position("G") == 0                        # принудительно закрыта
    assert b.equity(crash) >= 0                         # закрыта, пока equity ещё положительный
