...
python -m pytest tests/ -q               # 208 pytest (бэктестер + offline infra; 0 xfail — баги движка исправлены)
python -m unittest test_trading -v       # 8 регресс-тестов торгового слоя lab/+daybot (офлайн, всё замокано)
python -m unittest test_forward -v       # 24 теста forward-слоя lab/forward.py (офлайн, моки)
...
Ключевая ставка ЦБ — **14.25%** (снижена 2026-06-19). Источник правды —
`scripts/keyrate.py`; скрипты импортируют `KEYRATE`, а не держат локальные дубли.
...
tests/         — pytest-тесты бэктестера и offline-инфраструктуры; запуск из корня (conftest делает sys.path-хак).
