...
1. ПУТ-КОЛЛ ПАРИТЕТ (futures-style, без дисконта): F_impl(K) = K + C(K) − P(K) должен быть
   ОДИНАКОВ для всех страйков K. Опционы FORTS маржируемые → паритет без дисконтирования.
   Разброс F_impl по страйкам = прямая мера согласованности теор-цен.
